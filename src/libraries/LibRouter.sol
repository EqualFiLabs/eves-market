...
library LibRouter {
    ...
    function assertNativeBalanceRestored(uint256 expectedBalance) internal view {
        uint256 balance = address(this).balance;
        if (balance != expectedBalance) {
            revert IVaultRouter.ResidualRouterNativeBalance(expectedBalance, balance);
        }
    }

    function assertERC1155BalanceRestored(address token, uint256 id, uint256 expectedBalance) internal view {
        uint256 balance = IERC1155(token).balanceOf(address(this), id);
        if (balance != expectedBalance) {
            revert IMakerLendingRouter.ResidualRouterPosition(id, balance);
        }
    }
